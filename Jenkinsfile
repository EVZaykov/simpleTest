pipeline{
	...
	stages {
		stage('Start Grid'){
			steps {
				bat 'Bocker-compose up -d selenium-hub && docker-compose scale $params.BROWSER=3'
			}
		}
		stage('Run Test'){
			steps{
				bat 'mvn clean test "-Dwebdriver.browser.name=$params.BROWSER" "-Dcucumber.options=--tags @qwert12"'
			}
		}
	}
	post{
